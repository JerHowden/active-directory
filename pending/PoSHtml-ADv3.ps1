...
.PARAMETER CompanyLogo
    ...
    -CompanyLogo "\\Server01\Admin\Files\CompanyLogo.png" or "C:\tmp\Novacoastlogo.png"

.PARAMETER RightLogo
    Enter URL or UNC path to your desired right-side logo for generated report.

    -RightLogo "https://www.novacoast.com/wp-content/uploads/ncilogo.png"
...
param (
       ...
       [String]$CompanyLogo = "https://s3.amazonaws.com/nci-website/uploads/72dpi-novacoast-Simple_n.png",
       ...
       [String]$RightLogo = "https://s3.amazonaws.com/nci-website/uploads/72dpi-novacoast-Simple_n.png",
...
$DefaultVar = $(Get-Variable).Name
$ProdServer = 'dc01.prod.ncidemo.com'
$ProdDomain = 'prod.ncidemo.com'
...
$AllUsers = Get-ADUser -Filter * -Properties WhenCreated,ProtectedFromAccidentalDeletion,LastLogon,EmailAddress -Server $ProdServer

$GPOs = Get-GPO -All -Domain $ProdDomain | Select-Object DisplayName, GPOStatus, ModificationTime, @{ Label = "ComputerVersion"; Expression = { $_.computer.dsversion } }, @{ Label = "UserVersion"; Expression = { $_.user.dsversion } }

<###########################
         Dashboard
############################>

Write-Host ("[" + (Get-Date).DateTime + "] Working on Dashboard Report...") -ForegroundColor Green

$dte = (Get-Date).AddDays(- $ADModNumber)

$ADObjs = Get-ADObject -Filter { whenchanged -gt $dte -and ObjectClass -ne "domainDNS" -and ObjectClass -ne "rIDManager" -and ObjectClass -ne "rIDSet" } -Properties * -Server $ProdServer
...
#Company Information
$ADInfo = Get-ADDomain -Server $ProdServer
$ForestObj = Get-ADForest -Server $ProdServer
$DomainControllerobj = Get-ADDomain -Server $ProdServer
...
#Get Domain Admins
$DomainAdminMembers = Get-ADGroupMember "Domain Admins" -Server $ProdServer
...
$DefaultComputersOU = (Get-ADDomain -Server $ProdServer).computerscontainer
$DefaultComputers = Get-ADComputer -Filter * -Properties ProtectedFromAccidentalDeletion,Modified,PasswordLastSet,OperatingSystem -SearchBase "$DefaultComputersOU" -Server $ProdServer
...
$DefaultUsersOU = (Get-ADDomain -Server $ProdServer).UsersContainer
...
#Expiring Accounts
$LooseUsers = Search-ADAccount -AccountExpiring -UsersOnly -Server $ProdServer
...
#Security Logs
$SecurityLogs = Get-EventLog -Newest 7 -LogName "Security" -ComputerName $ProdServer | Where-Object { $_.Message -like "*An account*" }
...
#Tenant Domain
$Domains = Get-ADForest -Server $ProdServer | Select-Object -ExpandProperty upnsuffixes | ForEach-Object{
...
Write-Host ("[" + (Get-Date).DateTime + "] Working on Groups Report...") -ForegroundColor Green

#Get groups and sort in alphabetical order
$Groups = Get-ADGroup -Filter * -Properties mail,ProtectedFromAccidentalDeletion,managedBy -Server $ProdServer
$SecurityCount = 0
$MailSecurityCount = 0
$CustomGroup = 0
$DefaultGroup = 0
...
$GroupErrors = New-Object 'System.Collections.Generic.List[System.Object]'
$gProg = 1
foreach ($Group in $Groups)
{
       Write-Progress -Activity ("Processing Group #" + $gProg + " of " + $Groups.Count) -Status $group.Name -PercentComplete ($gProg / $groups.Count*100)
       $DefaultADGroup = 'False'
       $Type = New-Object 'System.Collections.Generic.List[System.Object]'
       $Gemail = (Get-ADGroup $Group -Properties mail -Server $ProdServer).mail
       ...
       if (($Group.Name -ne "Domain Users") -and ($Group.Name -ne "Domain Computers"))
       {
           Try {   
              $Users = (Get-ADGroupMember -Identity $Group -ErrorAction Stop -Server $ProdServer | Sort-Object DisplayName | Select-Object -ExpandProperty Name) -join ", "
             ...
             $GroupErrors += $Error[0].TargetObject
           }
       }
       ...
       $OwnerDN = Get-ADGroup -Filter { name -eq $Group.Name } -Properties managedBy -Server $ProdServer | Select-Object -ExpandProperty ManagedBy
       <#Try
       {
...
Write-Host ("[" + (Get-Date).DateTime + "] Working on Organizational Units Report...") -ForegroundColor Green

#Get all OUs'
$OUs = Get-ADOrganizationalUnit -Filter * -Properties * -Server $ProdServer
$OUwithLinked = 0
$OUwithnoLink = 0
$OUProtected = 0
$OUNotProtected = 0
...
foreach ($OU in $OUs)
{
       Write-Progress -Activity ("Processing OU #" + $oProg + " of " + $OUs.Count) -Status $OU.Name -PercentComplete ($oProg / $OUs.Count*100)
       ...
       else
       {
              ...
              foreach ($GPOlink in $GPOslinks)
              {
                     ...
                     $LinkedGPOs.Add((Get-GPO -Guid $Split2 -ErrorAction SilentlyContinue -Domain $ProdDomain).DisplayName)
              }
       }
...
Write-Host ("[" + (Get-Date).DateTime + "] Working on Users Report...") -ForegroundColor Green
...
{
       Write-Progress -Activity ("Processing User #" + $uProg + " of " + $AllUsers.Count) -Status $User.Name -PercentComplete ($uProg / $AllUsers.Count*100)
       ...
       $maxPasswordAge = (Get-ADDefaultDomainPasswordPolicy -Server $ProdServer).MaxPasswordAge.Days
       ...
       if ((($AttVar.PasswordNeverExpires) -eq $False) -and (($AttVar.Enabled) -ne $false))
       {
              ...
              else
              {
                     ...
                     $PasswordPol = (Get-ADUserResultantPasswordPolicy $user -Server $ProdServer)
...
############################>
Write-Host ("[" + (Get-Date).DateTime + "] Working on Group Policy Report...") -ForegroundColor Green
...
############################>
Write-Host ("[" + (Get-Date).DateTime + "] Working on Computers Report...") -ForegroundColor Green

$Computers = Get-ADComputer -Filter * -Properties ProtectedFromAccidentalDeletion,Modified,PasswordLastSet,OperatingSystem -Server $ProdServer
...
{
       Write-Progress -Activity ("Processing Computer #" + $cProg + " of " + $Computers.Count) -Status $Computer.Name -PercentComplete ($cProg / $Computers.Count*100)
...
Write-Host ("[" + (Get-Date).DateTime + "] Compiling Report...") -ForegroundColor Green
...
Write-Host ("[" + (Get-Date).DateTime + "] Publishing Final Report...") -ForegroundColor Green
#Dashboard Report
...
$FinalReport.Add($(Get-HTMLPieChart -ChartObject $PieObjectComputerObjOS -DataSet ($GraphComputerOS | ? {$_.Count -ne 0})))
$FinalReport.Add($(Get-HTMLContentclose))
...
#$ReportName = ("AD Report - $Day.$Month.$Year - $Hour.$Minute.$Second")
$ReportName = ("AD Report - $TimeStamp")
Save-HTMLReport -ReportContent $FinalReport -ShowReport -ReportName $ReportName -ReportPath $ReportSavePath 

#((Compare-Object -ReferenceObject (Get-Variable).Name -DifferenceObject $DefaultVar).InputObject) | % {Remove-Variable -Name $_ -Verbose -WhatIf} 
